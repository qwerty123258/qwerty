		...
		if (rncheck == "") {
			...
			$.ajax({
				...
				success : function(result) {
					if (result == "yes") {
						alert('숙소 수정 성공');
						location.href = "myRoomsList?page=1";
					} else {
						alert('숙소 수정 실패');
					}
...
<table>
	<input type="hidden" name="rno" id="rno" value="${room.rno}">
		<div id="deleteAr">
	...
	<tr>
	<td>숙소이름:</td>
	<td><input type="text" name="rname" id="rname" value="${room.rname}"></td>
	</tr>
	<tr>
	<td> 가격:</td>
	<td><input type="text" name="price" id="price" value="${room.price}"></td>
	</tr>
	<tr>
	<td>숙소 설명 </td>
	<td><textarea id="contents" name="contents" onkeyup="contentlength()" rows="15" cols="100">${room.contents}</textarea>
			...
			<tr>
			<td> 숙소사진:</td>
			<td><input type="file" multiple="multiple" name="pic"></td></tr>
			<tr>
			<td><h3>이미지 미리보기</h3></td><td>
<div id="imagePreview">
	<img id="img" />
</div>
</td>
...
<td>기존 파일</td>
<td>
	<c:forEach var="roomPic" items="${roomPic}">
					<div id="${roomPic.rpno}">
					${roomPic.rimgoriname}
					<button type="button" onclick="FileDelete('${roomPic.rimgname}',${roomPic.rpno})">삭제</button>
					</div>
			</c:forEach>
</td>
</tr>
...
<button type="button" class="btn btn-success" onclick="roomsUpdate()">수정 하기</button>
</td>
</tr>
</table>
</form>
