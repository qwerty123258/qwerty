	...
	public class DBsql {
		...
		public void TurnRepeat() { // 2번 커맨드 누르면 실행이며 반복됨.
			...
			String userA="A"; //USER A 이름 변수
			String userB="B"; //USER B 이름 변수
			...
			locationA=TurnA(locationA); //A의턴을 진행.
			...
			locationB=TurnB(locationB); //B의 턴을 진행
			if(!this.start) { //현재 필드의 값이 false이면 게임이 끝난것이므로
				rollback(); //값이 바뀌어버린 DB값을 롤백
				DisConnection(); //접속을 종료함.
				break; //반복 종료.
		...
		public int TurnA(int locationA) { //A의 턴을 진행해주는 메소드
			String name = "A";
			System.out.println("");
			...
			if (locationA >= 13) { // 최대 이동거리는 12인데 현재위치가 13보다 크거나 같아지는 경우
				...
				locationA = locationA % 12; // 현재위치를 12로 나눈 나머지로 바꿈.ex)현위치가 11일 주사위 3이나오면 다음 위치는 14인데 14%12를 하면 현재위치는
											// 2가 됨.
				ACityproPertyCheck(locationA, "A"); // A유저가 도시에 도착했을시 도시가 공백지인지,자신의 도시인지,타인의 도시인지 판단해주는 메소드.
				return locationA;
			} else {
				ACityproPertyCheck(locationA, "A");// A유저가 도시에 도착했을시 도시가 공백지인지,자신의 도시인지,타인의 도시인지 판단해주는 메소드.
			}
		...
		public int TurnB(int locationB) { //B의 턴을 진행해주는 메소드
			String othername = "B";
			System.out.println("");
			...
			if (locationB >= 13) {
				...
				BCityproPertyCheck(locationB, "B");// B유저가 도시에 도착했을시 도시가 공백지인지,자신의 도시인지,타인의 도시인지 판단해주는 메소드.
				return locationB;
			} else {
				BCityproPertyCheck(locationB, "B");// B유저가 도시에 도착했을시 도시가 공백지인지,자신의 도시인지,타인의 도시인지 판단해주는 메소드.
			}
		...
		public void ACityproPertyCheck(int locationA, String user) {// A유저가 도시에 도착했을시 도시가 공백지인지,자신의 도시인지,타인의 도시인지 판단해주는 메소드.
			String sql = "SELECT CITY,PRICE,PROPERTY FROM MONOPOLY WHERE CITYNO=?";
			try {
				if (user.equals("A")) { //A유저가 도시에 도착하였을때
					userSave = "B"; //상대 유저를 B라고 지정
				} else {
					userSave = "A"; //반대의 경우는 A라고 지정.
				}
				...
				while (rs.next()) {
					...
					if (locationA == 5) { //국세청이므로 세금을 내는 분기점.
						...
						UserMoneyTax(saveMoney, user); //A가 5번에 위치했을때 세금을 납부.
					} 
					else if ((rs.getString("property")).equals(user)) { //A의 도시에 방문하였을때 분기점.
						...
						UserMoneyPayment(saveMoney, locationA, user);
		...
		public void BCityproPertyCheck(int locationB, String user) {// B유저가 도시에 도착했을시 도시가 공백지인지,자신의 도시인지,타인의 도시인지 판단해주는 메소드.
			String sql = "SELECT CITY,PRICE,PROPERTY FROM MONOPOLY WHERE CITYNO=?";
			try {
				if (user.equals("B")) {// @
					userSave = "A";
				...
				while (rs.next()) {
					...
					if (locationB == 5) {
						...
						System.out.println(rs.getString("property") + "의 소유지 입니다. " + saveMoney + "를 지불합니다.");
						// 세금메소드(rs.getInt("price"))
						UserMoneyPayment(saveMoney, locationB, user);
					} else {
						System.out.println(rs.getString("city")+"에 방문했습니다.");
						System.out.println("이 " +rs.getString("city")+"는 소유한 사람이없습니다.  가격: " + saveMoney);
						System.out.println("");
		...
		public void UserMoneyPayment(int price, int locationA, String user) {// 상대에서 도시의 가격만큼 값을 지불하는 메소드
			...
			try {
				...
				if (saveMoney <= price) {// 현재 잔액보다 지불금액이 높거나 같다면 파산하고 게임이 종료되는 분기점
					if (user.equals("A")) { //만약 위에서 A가 지불을 했다면 B는 돈을 받아야하므로 A와 B를 전환.
						userSave = "B";
					} else {
						userSave = "A"; //반대로 B가 지불을 했다면 B와 A를 전환.
					}
					System.out.println(user+"님이 파산하셨습니다");
					...
					int saveMoney=UserMoneySearch(user);
					if(saveMoney >= price) {
		...
		public void UserMoneyBonus(String user) {// 출발지점을 지나면 추가 월급을 제공해주는 메소드
		...
		public void UserMoneyTax(int price, String user) {// 세금 메소드(가격,지불하는계정)
			String sql = "SELECT MONEY FROM USERLIST WHERE NAME=?";
			try {
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, user);
		...
		public void CityPurchase(int price, String user,int locationA) { //도시를 구매하는 메소드
			String sql = "SELECT MONEY FROM USERLIST WHERE NAME=?";
			try {
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, user);
